  ...
  const recentBookings = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from("bookings")
        .select("id, ref_code, status, starts_at, amount_cents, currency, customers(display_name), services(name)")
    ...
    <div className="space-y-6 max-w-7xl mx-auto w-full">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Hello {me?.user.full_name?.split(" ")[0] ?? "there"} — here's what's happening at {me?.tenant.name}.</p>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          ...
          <CardContent className="p-0">
            <Table>
              ...
              <TableBody>
                ...
                {recentBookings.data?.length === 0 && <TableRow><TableCell colSpan={6} className="text-center text-sm text-muted-foreground py-8">No bookings yet</TableCell></TableRow>}
              </TableBody>
            </Table>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="p-0">
            <Table>
              <TableHeader><TableRow><TableHead>Ref</TableHead><TableHead>Gateway</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Amount</TableHead><TableHead>Paid</TableHead></TableRow></TableHeader>
              <TableBody>
                {(recentPayments.data ?? []).map((p: any) => (
                  <TableRow key={p.id}>
                    ...
                    <TableCell className="text-right">{formatCurrency(p.amount_cents, p.currency)}</TableCell>
                    <TableCell className="text-xs">{p.paid_at ? relativeTime(p.paid_at) : "—"}</TableCell>
                  </TableRow>
                ))}
                {recentPayments.data?.length === 0 && <TableRow><TableCell colSpan={5} className="text-center text-sm text-muted-foreground py-8">No payments yet</TableCell></TableRow>}
              </TableBody>
            </Table>
          </CardContent>
        </Card>
...
function KpiCard({ label, value, icon: Icon }: { label: string; value: React.ReactNode; icon: React.ComponentType<{ className?: string }> }) {
  return (
    <Card>
...
export function StatusBadge({ status }: { status: string }) {
  ...
  return <Badge className={`${cls} border-0 font-medium text-[11px] uppercase tracking-wide`}>{status.replace(/_/g, " ").toLowerCase()}</Badge>;
}
