    ...
    <div className="space-y-4">
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            <div className="text-sm font-medium ml-2">
              ...
              {new Date(anchor.getTime() + 6 * 86400000).toLocaleDateString(undefined, {
                month: "short",
                day: "numeric",
                year: "numeric",
              })}
            ...
            <div className="grid grid-cols-[60px_repeat(7,minmax(140px,1fr))] min-w-[1000px]">
              ...
              {days.map((d) => {
                ...
                return (
                  <div
                    key={d.toISOString()}
                    className={`border-b border-r p-2 text-center ${isToday ? "bg-primary/5" : "bg-muted/30"}`}
                  >
                    <div className="text-[10px] uppercase text-muted-foreground">
                      {d.toLocaleDateString(undefined, { weekday: "short" })}
                    </div>
                    <div
                      className={`text-lg font-display ${isToday ? "text-primary font-semibold" : ""}`}
                    >
                      {d.getDate()}
                    </div>
                  </div>
              ...
              {HOURS.map((h) => (
                <>
                  <div
                    key={`h-${h}`}
                    className="border-b border-r text-[10px] text-muted-foreground text-right pr-2 pt-1 h-20"
                  >
                    {h % 12 === 0 ? 12 : h % 12}
                    {h < 12 ? "am" : "pm"}
                  </div>
                  {days.map((d) => {
                    ...
                    return (
                      <div
                        key={`${d.toISOString()}-${h}`}
                        className="border-b border-r h-20 p-1 space-y-1 relative"
                      >
                        {cellBookings.map((b: any) => (
                          <div
                            key={b.id}
                            className="text-[10px] rounded bg-primary/10 border-l-2 border-primary px-1.5 py-1 truncate"
                          >
      ...
      <div className="flex items-center gap-3 text-xs text-muted-foreground">
        ...
        <Badge variant="outline" className="ml-auto">
          Two-way Google / Outlook sync coming in next phase
        </Badge>
      </div>
    </div>
  );
