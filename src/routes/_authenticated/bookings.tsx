...
function BookingsPage() {
  ...
  const openEdit = (b: any) => {
    setForm({
      ...
      duration_minutes: Math.max(
        15,
        Math.round((new Date(b.ends_at).getTime() - new Date(b.starts_at).getTime()) / 60000),
      ),
  ...
  const confirmDelete = (id: string, ref?: string) => {
    if (
      ...
      window.confirm(`Delete booking ${ref ?? ""}? This cannot be undone.`)
    ) {
      deleteMut.mutate(id);
    }
  };
                ...
                >
                  ...
                  <TableCell className="text-right" onClick={(e) => e.stopPropagation()}>
                    <div className="flex items-center justify-end gap-1.5">
                      <Select
                        value={b.status}
                        onValueChange={(v) => setStatusMut.mutate({ id: b.id, status: v as any })}
                      >
            ...
            <div className="grid grid-cols-[1fr_120px] gap-3">
              <div>
                ...
                <Input
                  ...
                  min={0}
                  step="0.01"
                  value={(form.amount_cents / 100).toString()}
                  onChange={(e) =>
                    setForm({ ...form, amount_cents: Math.round(Number(e.target.value) * 100) })
                  }
                />
              </div>
              <div>
