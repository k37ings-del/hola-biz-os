...
function ReschedulePage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => fetchFn({ data: { token, kind: "reschedule" } }),
  });
        ...
        <CardContent className="space-y-4">
          <div className="rounded-lg bg-muted/40 p-3 text-sm">
            ...
            <div className="text-muted-foreground">
              Currently: {new Date((data as any).starts_at).toLocaleString()}
            </div>
          </div>
          {done ? (
            <div className="flex items-center gap-2 text-sm text-green-600">
              <CheckCircle2 className="h-4 w-4" /> Your appointment has been moved.
            </div>
          ) : (
            <>
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  try {
                    await rescheduleFn({
                      data: { token, new_starts_at: new Date(when).toISOString() },
                    });
                    setDone(true);
                    router.invalidate();
                  } catch (e: any) {
