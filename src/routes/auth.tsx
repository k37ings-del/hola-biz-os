    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-background via-secondary/30 to-accent/40 px-4 py-12">
      <div className="w-full max-w-xl">
        <div className="flex items-center gap-3 justify-center mb-8">
          ...
          <span className="font-subhead text-sm uppercase tracking-[0.18em] text-muted-foreground">
            Business OS
          </span>
        </div>
    ...
    try {
      ...
      const { data: signUp, error: sErr } = await supabase.auth.signUp({
        ...
        options: {
          ...
          data: { full_name: d.step2.full_name },
        },
      });
      if (sErr) throw sErr;
...
function Step1({
  ...
  defaults?: z.infer<typeof step1Schema>;
  onNext: (v: z.infer<typeof step1Schema>) => void;
}) {
  const form = useForm<z.infer<typeof step1Schema>>({
    resolver: zodResolver(step1Schema),
    defaultValues: defaults ?? { business_name: "", industry: "", country_code: "" },
  });
  return (
    <form onSubmit={form.handleSubmit(onNext)} className="space-y-4">
      <div className="space-y-1.5">
        ...
        {form.formState.errors.business_name && (
          <p className="text-xs text-danger">{form.formState.errors.business_name.message}</p>
        )}
      </div>
      <div className="space-y-1.5">
      ...
      <div className="space-y-1.5">
        ...
        <Select
          ...
          onValueChange={(v) => form.setValue("country_code", v, { shouldValidate: true })}
        >
          <SelectTrigger>
        ...
        {form.formState.errors.country_code && (
          <p className="text-xs text-danger">{form.formState.errors.country_code.message}</p>
        )}
      </div>
...
function Step2({
  ...
  defaults?: z.infer<typeof step2Schema>;
  onBack: () => void;
  onNext: (v: z.infer<typeof step2Schema>) => void;
}) {
  const form = useForm<z.infer<typeof step2Schema>>({
    resolver: zodResolver(step2Schema),
    defaultValues: defaults ?? { full_name: "", email: "", password: "", confirm: "" },
  });
  return (
    <form onSubmit={form.handleSubmit(onNext)} className="space-y-4">
      <div className="space-y-1.5">
...
function Step3({
  ...
  defaults?: z.infer<typeof step3Schema>;
  onBack: () => void;
  onNext: (v: z.infer<typeof step3Schema>) => void;
}) {
  const form = useForm<z.infer<typeof step3Schema>>({
    resolver: zodResolver(step3Schema),
    defaultValues: defaults ?? { whatsapp_number: "", wa_number_id: "" },
  });
  return (
    <form onSubmit={form.handleSubmit(onNext)} className="space-y-4">
      <p className="text-xs text-muted-foreground">
        Find these in Meta Business Manager → WhatsApp → API Setup. You can add this later in
        Settings.
      </p>
      <div className="space-y-1.5">
        <Label>WhatsApp Business Number</Label>
        <Input placeholder="+234..." {...form.register("whatsapp_number")} />
      </div>
      <div className="space-y-1.5">
        <Label>WhatsApp Number ID</Label>
        <Input placeholder="123456789012345" {...form.register("wa_number_id")} />
      </div>
      <div className="flex gap-2">
        <Button type="button" variant="outline" onClick={onBack}>
          <ArrowLeft className="h-4 w-4" /> Back
...
function Step4({
  ...
  defaults: typeof DEFAULT_BUSINESS_HOURS;
  onBack: () => void;
  onNext: (v: typeof DEFAULT_BUSINESS_HOURS) => void;
}) {
  ...
  return (
    <div className="space-y-4">
          ...
          return (
            <div key={key} className="grid grid-cols-12 gap-2 items-center py-1.5">
              <Switch
                ...
                checked={d.active}
                onCheckedChange={(v) => setHours({ ...hours, [key]: { ...d, active: v } })}
              />
              ...
              <Input
                ...
                value={d.open}
                disabled={!d.active}
                onChange={(e) => setHours({ ...hours, [key]: { ...d, open: e.target.value } })}
              />
              <Input
                className="col-span-4"
                type="time"
                value={d.close}
                disabled={!d.active}
                onChange={(e) => setHours({ ...hours, [key]: { ...d, close: e.target.value } })}
              />
            </div>
          );
        })}
      ...
      <div className="flex gap-2">
        ...
        <Button type="button" className="flex-1" onClick={() => onNext(hours)}>
          Continue <ArrowRight className="h-4 w-4" />
        </Button>
      </div>
    </div>
...
function Step5({
  ...
  country: string;
  defaults?: z.infer<typeof step5Schema>;
  submitting: boolean;
  onBack: () => void;
  onNext: (v: z.infer<typeof step5Schema>) => void;
}) {
  const { symbol, currency } = getCurrencyForCountry(country);
  const form = useForm<z.infer<typeof step5Schema>>({
    resolver: zodResolver(step5Schema),
    defaultValues: defaults ?? { service_name: "", price: 0 },
  });
  return (
    <form onSubmit={form.handleSubmit(onNext)} className="space-y-4">
      <p className="text-xs text-muted-foreground">
      ...
      <div className="space-y-1.5">
        ...
        <Input {...form.register("service_name")} placeholder="e.g. Consultation" />
        {form.formState.errors.service_name && (
          <p className="text-xs text-danger">{form.formState.errors.service_name.message}</p>
        )}
      </div>
      <div className="space-y-1.5">
        <Label>
