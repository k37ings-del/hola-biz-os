    ...
    <div className="min-h-screen grid place-items-center px-4 text-center">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-2">
          This waitlist link is invalid or has expired.
        </p>
      </div>
    </div>
  ),
...
function WaitlistClaim() {
  ...
  const mut = useMutation({
    ...
    onSuccess: (r) => {
      toast.success(`Booked! Ref ${r.ref_code}`);
      refetch();
    },
      ...
      <div className="max-w-lg mx-auto">
        <div className="mb-6 text-center">
          {data.tenant.logo_url && (
            <img src={data.tenant.logo_url} alt="" className="h-12 mx-auto mb-3" />
          )}
          <h1 className="text-xl font-display font-semibold">
            A spot opened up at {data.tenant.name}
          </h1>
          <p className="text-sm text-muted-foreground">
            Hi {data.customer_name}, you're next in line.
          </p>
        </div>
        <Card>
          <CardContent className="p-6 space-y-4">
            <div className="rounded-md bg-muted/50 p-4 space-y-2">
              ...
              <div className="text-sm text-muted-foreground">
                {data.service.duration_minutes} min ·{" "}
                {formatCurrency(data.service.price_cents, data.service.currency)}
              </div>
            </div>
            <div className="rounded-md border border-primary/30 bg-primary/5 p-4">
              ...
              {data.expires_at && !expired && status === "notified" && (
                <div className="text-xs text-muted-foreground mt-2 flex items-center gap-1">
                  <Clock className="h-3 w-3" /> Hold expires{" "}
            ...
            {status === "booked" ? (
              ...
              <div className="text-center text-sm text-destructive">
                This offer has expired. You're still on the waitlist for the next opening.
              </div>
            ) : status === "notified" ? (
              <Button
                className="w-full"
