...
function PortalPage() {
  ...
  const upcoming = (history as any[]).filter(
    (h) => new Date(h.starts_at) >= new Date() && h.status !== "CANCELLED",
  );
  const past = (history as any[]).filter(
    (h) => new Date(h.starts_at) < new Date() || h.status === "CANCELLED",
  );
      ...
      <main className="max-w-3xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="rounded-xl border bg-card p-5 shadow-sm">
          <div className="flex items-start justify-between gap-3">
            ...
            <Badge variant="outline" className={statusTone[booking.status] ?? ""}>
              {booking.status.replace("_", " ")}
            </Badge>
          </div>
          <h2 className="font-display font-semibold text-2xl mt-3">
            Hi {booking.customer_name.split(" ")[0]} 👋
          </h2>
          <p className="text-sm text-muted-foreground">
            Here's everything about your appointment with {tenant.name}.
          </p>
          ...
          {booking.status !== "CANCELLED" && booking.status !== "COMPLETED" && (
            <div className="mt-5 flex flex-wrap gap-2">
              <Button asChild variant="outline" size="sm">
                <Link to="/reschedule/$token" params={{ token: booking.reschedule_token }}>
                  Reschedule
                </Link>
              </Button>
              <Button
          ...
          <Section title="Other upcoming appointments">
            <div className="space-y-2">
              {upcoming
                ...
                .map((h) => (
                  <HistoryRow key={h.id} h={h} statusTone={statusTone} />
                ))}
            </div>
          </Section>
        )}
