...
const WeeklySchema = z.record(DayKey, z.object({
  active: z.boolean(),
...
export const listSchedules = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [{ data: staff }, { data: schedules }] = await Promise.all([
      context.supabase.from("staff").select("id, name, role, photo_url, ics_token, active").eq("tenant_id", tenantId).eq("active", true).order("name"),
...
export const upsertSchedule = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: unknown) => z.object({
    ...
    weekly: WeeklySchema,
    time_off: z.array(z.object({ from: z.string(), to: z.string(), reason: z.string().optional() })).default([]),
