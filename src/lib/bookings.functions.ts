...
export const listBookings = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    if (!tenantId)
      return { bookings: [], stats: { upcoming: 0, today: 0, pending: 0, completed_week: 0 } };

    const { data, error } = await context.supabase
      .from("bookings")
      .select(
        "id, ref_code, status, starts_at, ends_at, amount_cents, currency, notes, cancellation_reason, no_show_reason, customer:customers(id, display_name, wa_phone), service:services(id, name), staff:staff(id, name)",
      )
    ...
    return {
      ...
      stats: {
        ...
        completed_week: list.filter(
          (b: any) => b.status === "COMPLETED" && new Date(b.starts_at).getTime() > weekAgo,
        ).length,
      },
    };
  });
